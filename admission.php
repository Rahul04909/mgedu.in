 ...
 * Premium Dynamic Course Admission Portal - Student Enrollment Console
 */
...
// 1. Handle AJAX Requests (OTP & Payment Verification)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($_POST['ajax_action'] === 'send_otp') {
        ...
        $subject = "Your Admission Email Verification OTP";
        $body = "
<div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
    ...
    <div style='padding: 30px; background: #ffffff; text-align: center;'>
        ...
        <p style='color: #475569; font-size: 15px; line-height: 1.6;'>Thank you for starting your admission profile. To securely verify your email address, please use the One-Time Password (OTP) below:</p>
    ...
    if ($_POST['ajax_action'] === 'verify_payment') {
        ...
        try {
            ...
            $stmtDetails = $db->prepare("SELECT id, student_name, email, password FROM `admissions` WHERE `razorpay_order_id` = ?");
            ...
            if ($admission) {
                ...
                $stmtSeq = $db->prepare("SELECT enrollment_number FROM admissions WHERE enrollment_number LIKE ? ORDER BY id DESC LIMIT 1");
                ...
                $stmt = $db->prepare("UPDATE `admissions` SET `payment_status` = 'paid', `status` = 'confirmed', `razorpay_payment_id` = ?, `enrollment_number` = ? WHERE `razorpay_order_id` = ?");
                $stmt->execute([$razorpay_payment_id, $enrollment_number, $razorpay_order_id]);
                
                // Generate premium tax receipt PDF
                $receiptFile = MG_GenerateReceiptPDF($admission['id']);
                
                // Sync with student_fees table
                try {
                    $relativeReceipt = 'assets/uploads/admissions/receipts/receipt_' . $admission['id'] . '.pdf';
                    
                    // Fetch amount from admissions
                    $stmtC = $db->prepare("SELECT sales_price FROM `admissions` WHERE id = ?");
                    ...
                    if ($feeCheck->fetch()) {
                        $feeUpd = $db->prepare("UPDATE `student_fees` SET enrollment_number = ?, amount = ?, payment_status = 'paid', receipt_path = ?, razorpay_payment_id = ? WHERE student_id = ? AND student_type = 'course'");
                        $feeUpd->execute([$enrollment_number, $amount, $relativeReceipt, $razorpay_payment_id, $admission['id']]);
                    } else {
                        $feeIns = $db->prepare("INSERT INTO `student_fees` (student_id, student_type, enrollment_number, amount, payment_status, receipt_path, razorpay_payment_id) VALUES (?, 'course', ?, ?, 'paid', ?, ?)");
                        $feeIns->execute([$admission['id'], $enrollment_number, $amount, $relativeReceipt, $razorpay_payment_id]);
                    }
                } catch (Exception $feeEx) {
                    error_log("Failed to sync course fee during Razorpay callback: " . $feeEx->getMessage());
                }
                
                $subject = "Admission Confirmed - MG Education";
                $body = "
                <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
                    ...
                    <div style='padding: 30px; background: #ffffff;'>
                        ...
                        <p style='color: #475569; font-size: 15px; line-height: 1.6;'>Your admission profile and fee payment have been successfully processed. We are thrilled to welcome you as our student.</p>
                        ...
                        <p style='color: #475569; font-size: 14px; line-height: 1.6; margin-top: 15px;'>Our academic counselor will contact you shortly with your class schedule and learning portal access instructions.</p>
                    </div>
                    <div style='background: #f1f5f9; padding: 15px; text-align: center; border-top: 1px solid #e2e8f0;'>
                        <p style='color: #94a3b8; font-size: 12px; margin: 0;'>&copy; " . date('Y') . " MG Education & Social Development Organization. All Rights Reserved.</p>
                    </div>
            ...
            $_SESSION['admission_success_message'] = "Payment successful! Admission confirmed.";
            ...
            try {
                $db = MG_GetDBConnection();
                $stmt = $db->prepare("UPDATE `admissions` SET `payment_status` = 'failed' WHERE `razorpay_order_id` = ?");
...
// 2. Self-Healing Database Schema
try {
    $db->query("SELECT 1 FROM `admissions` LIMIT 1");
    // Ensure dob column exists
    try {
        $db->query("SELECT `dob` FROM `admissions` LIMIT 1");
    } catch (Exception $e) {
        $db->exec("ALTER TABLE `admissions` ADD COLUMN `dob` DATE NOT NULL AFTER `student_name`");
    }

    $newColumns = [
        'session_name'          => "VARCHAR(50) NULL AFTER `course_id`",
        'highest_qualification' => "VARCHAR(50) NULL AFTER `gender`",
        'school_name'           => "VARCHAR(255) NULL AFTER `highest_qualification`",
        'aadhaar_number'        => "VARCHAR(20) NULL AFTER `school_name`",
        'pan_number'            => "VARCHAR(20) NULL AFTER `aadhaar_number`",
        'aadhaar_card'          => "VARCHAR(255) NULL AFTER `aadhaar_number`",
        'class_10_roll'         => "VARCHAR(50) NULL AFTER `aadhaar_card`",
        'class_10_board'        => "VARCHAR(100) NULL AFTER `class_10_roll`",
        'class_10_school'       => "VARCHAR(255) NULL AFTER `class_10_board`",
        'class_10_marksheet'    => "VARCHAR(255) NULL AFTER `class_10_school`",
        'class_12_roll'         => "VARCHAR(50) NULL AFTER `class_10_marksheet`",
        'class_12_board'        => "VARCHAR(100) NULL AFTER `class_12_roll`",
        'class_12_school'       => "VARCHAR(255) NULL AFTER `class_12_board`",
        'class_12_marksheet'    => "VARCHAR(255) NULL AFTER `class_12_school`",
        'college_name'          => "VARCHAR(255) NULL AFTER `class_12_marksheet`",
        'college_roll'          => "VARCHAR(50) NULL AFTER `college_name`",
        'college_degree'        => "VARCHAR(100) NULL AFTER `college_roll`",
        'college_passing_year'  => "VARCHAR(10) NULL AFTER `college_degree`",
        'college_marks_type'    => "VARCHAR(20) NULL AFTER `college_passing_year`",
        'college_marks_value'   => "VARCHAR(20) NULL AFTER `college_marks_type`",
        'college_marksheet'     => "VARCHAR(255) NULL AFTER `college_marks_value`",
        'razorpay_order_id'     => "VARCHAR(255) NULL AFTER `status`",
        'razorpay_payment_id'   => "VARCHAR(255) NULL AFTER `razorpay_order_id`",
        'payment_status'        => "VARCHAR(50) DEFAULT 'pending' AFTER `razorpay_payment_id`",
        'enrollment_number'     => "VARCHAR(50) NULL AFTER `id`",
        'password'              => "VARCHAR(255) NULL AFTER `enrollment_number`"
    ];

    foreach ($newColumns as $col => $definition) {
        try {
            $db->query("SELECT `$col` FROM `admissions` LIMIT 1");
        } catch (Exception $e) {
            $db->exec("ALTER TABLE `admissions` ADD COLUMN `$col` $definition");
        }
    }
} catch (Exception $e) {
    try {
...
// 3. Fetch courses & sessions
$courses = [];
try {
    $course_stmt = $db->query("SELECT id, name, sales_price FROM courses ORDER BY name ASC");
    $courses = $course_stmt->fetchAll();
} catch (Exception $e) {
    error_log("Admissions course fetch error: " . $e->getMessage());
}
...
try {
    ...
    error_log("Admissions session fetch error: " . $e->getMessage());
}

// 4. Process Form Submission
$razorpay_order_id_global = null;
$course_sales_price_global = 0;
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_admission'])) {
    try {
        $course_id = intval($_POST['course_id'] ?? 0);
        $pan_number = null; // Removed from UI fields as requested
        ...
        if (empty($course_id) || empty($session_name) || empty($student_name) || empty($dob) || empty($email) || empty($mobile) || empty($gender) ||
        ...
        // Scanned PDF uploads setup
        $docDir = __DIR__ . '/assets/uploads/admissions/documents/';
        ...
        // Conditional Qualifications visibility logic
        ...
        $photoDir = __DIR__ . '/assets/uploads/admissions/photos/';
        $sigDir = __DIR__ . '/assets/uploads/admissions/signatures/';
        ...
        $stmt = $db->prepare("
            INSERT INTO `admissions` (
                course_id, session_name, student_name, dob, email, mobile, gender, 
        ...
        $course_sales_price = 0;
        foreach ($courses as $c) {
            if ($c['id'] == $course_id) {
                $course_sales_price = floatval($c['sales_price']);
                break;
            }
        }
        
        $course_sales_price_global = $course_sales_price;
        
        // Insert into student_fees table initially as pending
        try {
            $stmtFee = $db->prepare("INSERT INTO `student_fees` (student_id, student_type, enrollment_number, amount, payment_status, receipt_path, razorpay_payment_id) VALUES (?, 'course', 'PENDING', ?, 'pending', NULL, NULL)");
            $stmtFee->execute([$admission_id, $course_sales_price]);
        } catch (Exception $feeEx) {
            error_log("Failed to insert initial pending course fee record: " . $feeEx->getMessage());
        }

        $amount_in_paise = (int)($course_sales_price * 100);

        if ($amount_in_paise > 0) {
            $api = MG_GetRazorpayClient();
            $orderData = [
                'receipt'         => 'rcptid_' . $admission_id,
            ...
            $stmt = $db->prepare("UPDATE `admissions` SET `razorpay_order_id` = ? WHERE id = ?");
            ...
            $stmt = $db->prepare("UPDATE `admissions` SET `payment_status` = 'free', `status` = 'confirmed', `enrollment_number` = ? WHERE id = ?");
            $stmt->execute([$enrollment_number, $admission_id]);
            
            $receiptFile = MG_GenerateReceiptPDF($admission_id);
            ...
            try {
                ...
                if ($feeCheck->fetch()) {
                    $feeUpd = $db->prepare("UPDATE `student_fees` SET enrollment_number = ?, amount = 0.00, payment_status = 'free', receipt_path = ?, razorpay_payment_id = 'SCHOLARSHIP' WHERE student_id = ? AND student_type = 'course'");
                    $feeUpd->execute([$enrollment_number, $relativeReceipt, $admission_id]);
                } else {
                    $feeIns = $db->prepare("INSERT INTO `student_fees` (student_id, student_type, enrollment_number, amount, payment_status, receipt_path, razorpay_payment_id) VALUES (?, 'course', ?, 0.00, 'free', ?, 'SCHOLARSHIP')");
                    $feeIns->execute([$admission_id, $enrollment_number, $relativeReceipt]);
                }
            } catch (Exception $feeEx) {
                error_log("Failed to sync course fee during free registration: " . $feeEx->getMessage());
            }
            ...
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
                ...
                <div style='padding: 30px; background: #ffffff;'>
                    ...
                    <p style='color: #475569; font-size: 15px; line-height: 1.6;'>Your admission profile has been successfully processed. We are thrilled to welcome you as our student.</p>
                    <div style='background: #f8fafc; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0; margin: 25px 0; text-align: left;'>
                        <p style='margin: 0 0 12px 0; font-size: 13px; color: #475569; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #cbd5e1; padding-bottom: 6px;'>Your Student Portal Credentials</p>
                        <table style='width: 100%; border-collapse: collapse;'>
                            <tr>
                    ...
                    <p style='color: #475569; font-size: 14px; line-height: 1.6;'>Your official A4 admission receipt has been dynamically generated and attached to this email. Please keep your enrollment number and portal login password secure for online course access.</p>
                    <p style='color: #475569; font-size: 14px; line-height: 1.6; margin-top: 15px;'>Our academic counselor will contact you shortly with your class schedule and learning portal access instructions.</p>
                </div>
                <div style='background: #f1f5f9; padding: 15px; text-align: center; border-top: 1px solid #e2e8f0;'>
                    <p style='color: #94a3b8; font-size: 12px; margin: 0;'>&copy; " . date('Y') . " MG Education & Social Development Organization. All Rights Reserved.</p>
            ...
            $success_message = "Admissions profile submitted successfully! Your admission is confirmed.";
        }
...
$dynamic_title = "Online Course Admission Portal | MG Education";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="portal-wrapper">
    ...
    <div class="portal-sidebar">
        <div class="sidebar-header">
            <h1 class="sidebar-title">MG Education <span class="d-block text-warning" style="font-size: 20px;">Admission Portal</span></h1>
            <p class="sidebar-subtitle">Apply for online courses with valid academic credentials for immediate approval processing.</p>
        </div>
        ...
        <div class="sidebar-steps" id="formStepProgress">
            <div class="step-item active" id="step-1">
                ...
                <div>
                    ...
                    <span class="step-desc">Course & Personal Info</span>
                </div>
            </div>
    ...
    <div class="portal-content">
        <form method="POST" action="" enctype="multipart/form-data" id="admissionForm" novalidate>
            <div class="admission-form-body">
                ...
                <!-- SECTION 1: Course & Personal Info -->
                <div class="form-section-card">
                    <h3 class="section-card-title">
                        <i class="fa-solid fa-graduation-cap"></i> 01. Academic & Personal Specification
                    </h3>
                    <div class="section-card-body">
                        
                        <!-- Row 1: Course Selection | Session | Gender -->
                        <div class="row">
                            <div class="col-md-5 mb-4">
                                <label class="form-label" for="course_id">Course Selection <span class="text-danger">*</span></label>
                                <div class="field-icon-wrapper">
                                    <select name="course_id" id="course_id" class="form-control-premium form-select form-select-premium" required>
                                        <option value="">Choose Course</option>
                                        <?php foreach ($courses as $c): ?>
                                            <option value="<?= $c['id'] ?>" <?= ($selected_course_id == $c['id']) ? 'selected' : '' ?>><?= htmlspecialchars($c['name']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <i class="fa-solid fa-book-open input-icon"></i>
                                    <div class="invalid-feedback">Please select a valid academic course.</div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                <label class="form-label" for="student_name">Candidate's Name <span class="text-danger">*</span> <span class="label-helper-text">(As per 10th Certificate)</span></label>
                                <div class="field-icon-wrapper">
                                    <input type="text" name="student_name" id="student_name" class="form-control-premium" placeholder="e.g. Rahul Sen" required minlength="3">
                                    <i class="fa-solid fa-user input-icon"></i>
                                    <div class="invalid-feedback">Candidate name is required (min 3 chars).</div>
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                <label class="form-label" for="dob">Date of Birth <span class="text-danger">*</span> <span class="label-helper-text">(As per 10th Certificate)</span></label>
                                <div class="field-icon-wrapper">
                ...
                <!-- SECTION 2: Educational Qualification Profile -->
                <div class="form-section-card">
                    <h3 class="section-card-title">
                        <i class="fa-solid fa-user-graduate"></i> 02. Educational Qualification Profile
                    </h3>
                    <div class="section-card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <div class="field-icon-wrapper">
                                        ...
                                        <div class="invalid-feedback">Please enter a valid 12-digit Aadhaar.</div>
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-4">
                                ...
                                <div class="field-icon-wrapper">
                                    ...
                                    <div class="invalid-feedback">Highest academic qualification is required.</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="qual-10th-box" class="qualification-conditional-box">
                                ...
                                <div class="col-md-4 mb-4">
                                    ...
                                    <div class="field-icon-wrapper">
                                        <select id="class_10_board" name="class_10_board" class="form-control-premium form-select form-select-premium" required>
                                            ...
                                            <option value="Haryana Board">HBSE (Haryana Board)</option>
                                            <option value="Bihar Board">BSEB (Bihar Board)</option>
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-4">
                                    <label class="form-label">Upload Class 10th Marksheet <span class="text-danger">*</span> <span class="label-helper-text">(Scanned PDF scan under 2MB)</span></label>
                        ...
                        <div id="qual-12th-box" class="qualification-conditional-box">
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-4">
                                    <label class="form-label">Upload Class 12th Marksheet <span class="text-danger">*</span> <span class="label-helper-text">(Scanned PDF scan under 2MB)</span></label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var options = {
                ...
                "amount": "<?= (int)($course_sales_price_global * 100) ?>",
                "currency": "INR",
                "name": "MG Education",
                        ...
                        success: function(data) {
                            if (data.success) {
                                Swal.fire({
                                    ...
                                    text: 'Your admission profile has been registered and fee received.',
                                    confirmButtonColor: '#10b981'
                                }).then(() => {
                ...
                "modal": {
                    "ondismiss": function() {
                        Swal.fire({
                            ...
                            text: 'Your payment was cancelled. Your admission is pending payment.',
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                ...
                text: 'Your academic admissions profile has been registered in the MG Education registry.',
                confirmButtonColor: '#10b981',
