 ...
 * Franchise Portal - Manual Course Student Enrollment Wizard
 */
...
// 1. Handle Form Submission (Post Request Processing before Header to prevent output buffering issues)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_admission'])) {
    ...
    try {
        ...
        // Fetch selected course pricing
        $cStmt = $db->prepare("SELECT name, sales_price FROM courses WHERE id = ? LIMIT 1");
        $cStmt->execute([$course_id]);
        $courseRow = $cStmt->fetch();
        if (!$courseRow) {
            throw new Exception("Selected course is invalid or has been retired.");
        }
        $course_sales_price = floatval($courseRow['sales_price']);
        ...
        if ($balance < $course_sales_price) {
            throw new Exception("Insufficient wallet balance. Course enrollment fee: ₹" . number_format($course_sales_price, 2) . ", Current Balance: ₹" . number_format($balance, 2) . ". Please recharge your wallet first.");
        }
        ...
        try {
            ...
            ");
            $tStmt->execute([$center_id, -$course_sales_price, $debitTxnId, $remarks]);
            ...
            $stmtSeq = $db->prepare("SELECT enrollment_number FROM admissions WHERE enrollment_number LIKE ? ORDER BY id DESC LIMIT 1 FOR UPDATE");
            ...
            // 4. Write Core student registry profile to admissions table
            $stmt = $db->prepare("
                INSERT INTO `admissions` (
                    added_by, enrollment_number, course_id, session_name, student_name, dob, email, mobile, gender, 
            ...
            // 6. Compile A4 Tax Invoice Receipt via mPDF
            $receiptFile = MG_GenerateReceiptPDF($admission_id);
            $relativeReceipt = $receiptFile ? 'assets/uploads/admissions/receipts/receipt_' . $admission_id . '.pdf' : NULL;
            ...
            $feeIns = $db->prepare("
                ...
                VALUES (?, 'course', ?, ?, 'paid', ?, ?)
            ");
            $feeIns->execute([$admission_id, $enrollment_number, $course_sales_price, $relativeReceipt, 'WALLET_DEDUCT_' . $center_id]);
        ...
        $subject = "Admission & Enrollment Confirmation - MG Education";
        $body = "
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
            ...
            <div style='padding: 30px; background: #ffffff;'>
                ...
                <p style='color: #475569; font-size: 15px; line-height: 1.6;'>Congratulations! Your admission has been manually submitted and confirmed by " . $centerName . ". Your student profile is successfully created in our global registry database.</p>
                ...
                <div style='background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 20px; margin: 20px 0;'>
                    <table style='width: 100%; border-collapse: collapse; font-size: 14px;'>
                        ...
                        <tr>
                            ...
                            <td style='padding: 6px 0; color: #1e293b; font-weight: bold;'>" . htmlspecialchars($courseRow['name']) . "</td>
                        </tr>
                        <tr>
...
// Load active courses & sessions
$courses = [];
try {
    $cStmt = $db->query("SELECT id, name, sales_price FROM courses ORDER BY name ASC");
    $courses = $cStmt->fetchAll();
} catch (Exception $e) {
    error_log("Failed to load courses: " . $e->getMessage());
}
            ...
            <form id="enrollmentForm" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                ...
                <div class="form-step-section" id="section-1">
                    <div class="form-section-card">
                        <h3 class="section-card-title"><i class="fas fa-graduation-cap"></i> 1. Selected Academic Program</h3>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label font-weight-bold">Select Target Course <span class="text-danger">*</span></label>
                                <select name="course_id" id="course_id" class="form-select form-control" required onchange="updateCoursePrice(this)">
                                    <option value="">-- Choose Course --</option>
                                    <?php foreach ($courses as $c): ?>
                                        <option value="<?= $c['id'] ?>" data-price="<?= $c['sales_price'] ?>"><?= htmlspecialchars($c['name']) ?> (₹<?= number_format($c['sales_price'], 2) ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                    ...
                    <div class="form-section-card bg-light">
                        ...
                        <div class="p-3 border rounded bg-white">
                            <div class="d-flex justify-content-between font-weight-bold mb-2">
                                <span>Selected Course Enrollment Fee:</span>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        // Always trigger updateCoursePrice on load to set initial fee and balance status
        const courseSelect = document.getElementById('course_id');
        updateCoursePrice(courseSelect);
    });

    function updateCoursePrice(elem) {
    ...
    $('#enrollmentForm').on('submit', function(e) {
        ...
        const selectedCourseSelect = document.getElementById('course_id');
        const selectedOption = selectedCourseSelect.options[selectedCourseSelect.selectedIndex];
        const coursePrice = selectedOption ? parseFloat(selectedOption.getAttribute('data-price')) : 0;

        if (centerWalletBalance < coursePrice) {
            Swal.fire({
                icon: 'error',
        ...
        Swal.fire({
            ...
            text: 'Confirm enrolling candidate and deducting ₹' + coursePrice.toLocaleString('en-IN', { minimumFractionDigits: 2 }) + ' directly from center wallet?',
            icon: 'question',
