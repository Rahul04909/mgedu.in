...
// Route authorization guard: must have internship role and confirmed enrollment
if (!isset($_SESSION['student_role']) || $_SESSION['student_role'] !== 'internship' || empty($_SESSION['student_enrollment'])) {
    header("Location: ../index.php?error=unauthorized");
    exit();
}

$student_enrollment = $_SESSION['student_enrollment'];
$student_name = $_SESSION['student_name'] ?? 'Student';

// Check DB to ensure registration is active and confirmed
try {
    $db = MG_GetDBConnection();
    $stmt = $db->prepare("SELECT * FROM `internship_admissions` WHERE `enrollment_number` = ? AND `status` = 'confirmed'");
    $stmt->execute([$student_enrollment]);
    $student = $stmt->fetch();
    
    if (!$student) {
        // Revoke session if not found or no longer confirmed
    ...
    // Sync the student name if it was updated in the DB
...
$menuItems = [
    ...
    [
        "menuTitle" => "My Internship",
        "icon" => "fas fa-briefcase",
        "pages" => [
            ["title" => "Internship Details", "url" => "internship_details.php"]
        ],
    ],
    [
        "menuTitle" => "Settings",
...
<head>
    ...
    <style>
        ...
        /* Professional padding for content wrapper after breadcrumb removal */
        .content-wrapper {
            ...
            <div class="sidebar">
                <div class="user-panel mt-3 pb-3 mb-3">
                    <a href="./profile.php" class="d-flex align-items-center">
                        <div class="image">
                            <?php 
                                $sidebarPhoto = '../../' . htmlspecialchars($student['student_photo']);
                        ...
                        <div class="info">
                            <span class="d-block text-dark font-weight-bold" style="font-size: 13px;"><?= htmlspecialchars($student_name) ?></span>
                            <small class="text-secondary font-weight-bold" style="font-size: 11px; font-family: monospace; display: block; margin-top: 1px;"><?= htmlspecialchars($student_enrollment) ?></small>
                        </div>
                    </a>
                </div>
                <nav class="mt-2">
