 ...
 * Dedicated Internship Student Admission File Editor Dashboard
 */
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_admission') {
        ...
        if (empty($student_name) || empty($dob) || empty($email) || empty($mobile) || empty($internship_id)) {
            throw new Exception("Core fields (Name, Date of Birth, Email, Mobile, and Internship Program) are strictly mandatory.");
        }

        $update_sql = "
            UPDATE internship_admissions 
            SET 
                ...
                internship_id = :internship_id, enrollment_number = :enrollment_number, 
        ...
        if ($payment_status === 'paid' || $payment_status === 'free') {
            MG_GenerateInternshipReceiptPDF($id);
        }

        // Synchronize with student_fees table for internship
        try {
            ...
            // Fetch internship sales_price to get the exact amount
            $admCheck = $db->prepare("SELECT a.sales_price FROM internship_admissions a WHERE a.id = ?");
            $admCheck->execute([$id]);
            ...
            $relativeReceipt = ($payment_status === 'paid' || $payment_status === 'free') ? 'assets/uploads/internships/receipts/receipt_' . $id . '.pdf' : NULL;

            if ($exists) {
                $feeUpd = $db->prepare("UPDATE `student_fees` SET enrollment_number = ?, amount = ?, payment_status = ?, receipt_path = ? WHERE student_id = ? AND student_type = 'internship'");
                $feeUpd->execute([$enrollment_number ?: 'PENDING', $feeAmount, $payment_status, $relativeReceipt, $id]);
            } else {
                $feeIns = $db->prepare("INSERT INTO `student_fees` (student_id, student_type, enrollment_number, amount, payment_status, receipt_path) VALUES (?, 'internship', ?, ?, ?, ?)");
                $feeIns->execute([$id, $enrollment_number ?: 'PENDING', $feeAmount, $payment_status, $relativeReceipt]);
            }
        } catch (Exception $feeEx) {
            error_log("Failed to sync internship fee during admin edit: " . $feeEx->getMessage());
        }

        $success_message = "Student internship admission record updated successfully!";
        echo "<script>
    ...
    $stmt = $db->prepare("SELECT * FROM internship_admissions WHERE id = ?");
    $stmt->execute([$id]);
        ...
        <div class="col-12">
            <div class="edit-card">
                <div class="edit-header">
                    <h3><i class="fa-solid fa-user-pen mr-2"></i> Modify Internship Candidate Registry</h3>
                </div>
                ...
                <form action="" method="POST" class="mb-0">
                    ...
                    <div class="edit-body">
                        ...
                        <div class="edit-sect-title">Internship Registration & Credentials</div>
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label class="form-label-premium">Selected Internship Program</label>
                                <select name="internship_id" class="form-control form-control-premium form-select" required>
                                    <option value="">-- Choose Internship --</option>
                                    <?php foreach ($internships_list as $i): ?>
                                        <option value="<?= $i['id'] ?>" <?= ($admission['internship_id'] == $i['id']) ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="payment_status" class="form-control form-control-premium form-select">
                                    ...
                                    <option value="free" <?= ($admission['payment_status'] === 'free') ? 'selected' : '' ?>>Free / Scholarship</option>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control form-control-premium" name="enrollment_number" value="<?= htmlspecialchars($admission['enrollment_number'] ?? '') ?>" placeholder="e.g. MGINT0526001">
                            </div>
                            
                            <div class="col-md-6 mb-3">
                                <label class="form-label-premium">Student Portal Account Password</label>
                                <input type="text" class="form-control form-control-premium" name="password" value="<?= htmlspecialchars($admission['password'] ?? '') ?>" placeholder="Create password" required>
                            </div>
                        </div>
                    ...
                    <div class="card-footer bg-light border-0 px-4 py-3 d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary btn-action-pill" style="background-color: #0d47a1; border-color: #0d47a1;"><i class="fa-solid fa-floppy-disk mr-2"></i> Save Internship File</button>
                    </div>
                </form>
            </div>
        </div>
