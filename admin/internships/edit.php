...
$error_message = '';
$intern_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
$internship = null;

// Fetch Internship details
try {
    if ($intern_id <= 0) {
        throw new Exception("Invalid internship identifier.");
    }
    ...
    if (!$internship) {
        throw new Exception("Internship program record not found.");
    }
} catch (Exception $e) {
...
// Process Internship Update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_internship']) && $internship) {
    try {
        // Validate CSRF
        ...
        // Verify unique slug in other records
        $checkStmt = $db->prepare("SELECT id FROM internships WHERE slug = :slug AND id != :id LIMIT 1");
        $checkStmt->execute(['slug' => $slug, 'id' => $intern_id]);
        if ($checkStmt->fetch()) {
            throw new Exception("The slug '{$slug}' is occupied by another program. Please provide a unique slug.");
        }
        ...
        $internship_image = $internship['internship_image'];
        if (isset($_FILES['internship_image']) && $_FILES['internship_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!empty($internship['internship_image'])) {
                $oldCoverFile = $rootPath . '/' . $internship['internship_image'];
        ...
        $current_gallery = json_decode($internship['gallery_images'] ?? '[]', true);
                ...
                if ($files['error'][$key] === UPLOAD_ERR_OK) {
                    ...
                    if ($gInfo !== false) {
                        ...
                        if (move_uploaded_file($gTmp, $uploadFullDir . $newGName)) {
                            $current_gallery[] = 'assets/uploads/internships/' . $newGName;
                        }
                    }
                }
            }
        ...
        $brochure_pdf = $internship['brochure_pdf'];
        ...
        if ($brochure_enabled) {
            if (isset($_FILES['brochure_pdf']) && $_FILES['brochure_pdf']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!empty($internship['brochure_pdf'])) {
                    $oldPdfFile = $rootPath . '/' . $internship['brochure_pdf'];
        ...
        $featured_image = $internship['featured_image'];
        if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!empty($internship['featured_image'])) {
                $oldSeoFile = $rootPath . '/' . $internship['featured_image'];
        ...
        $ratingCount = $internship['ratings_count'] ?: rand(15, 60);
        $ratingSum = $internship['ratings_sum'] ?: ($ratingCount * 4.8);
        ...
        $stmt = $db->prepare("
            UPDATE internships SET 
                ...
                sales_price = :sales_price, internship_image = :internship_image, gallery_images = :gallery_images, 
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    text: 'The internship details have been updated successfully!',
    ...
    <?php if ($internship): ?>
        ...
        <div class="col-lg-8">
            <form method="POST" action="" enctype="multipart/form-data" id="internshipForm" onsubmit="return validatePricing()">
                ...
                <div class="admin-card">
                    ...
                    <div class="admin-body">
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" class="form-control" value="<?= htmlspecialchars($internship['name']) ?>" required autocomplete="off">
                        </div>

                        <div class="mb-4">
                            <label class="form-label" for="slug">URL Slug <span class="seo-indicator-badge">Auto / Customizable</span></label>
                            <div class="slug-wrapper">
                                <input type="text" name="slug" id="slug" class="form-control" value="<?= htmlspecialchars($internship['slug']) ?>" required readonly>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="category_id" id="category_id" class="form-control" required>
                                    ...
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?= $cat['id'] ?>" <?= $internship['category_id'] == $cat['id'] ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="mode" id="mode" class="form-control" required>
                                    <option value="offline" <?= $internship['mode'] === 'offline' ? 'selected' : '' ?>>Offline (Prayag Campus)</option>
                                    <option value="online" <?= $internship['mode'] === 'online' ? 'selected' : '' ?>>Online (Remote Live)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mb-0">
                            ...
                            <textarea name="description" id="description" class="form-control" rows="6" placeholder="Provide complete training details, project frameworks, and technologies covered..."><?= htmlspecialchars($internship['description'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <div class="d-flex gap-2">
                                    <input type="number" name="duration" id="duration" class="form-control" value="<?= htmlspecialchars($internship['duration']) ?>" min="1" required style="flex: 1.5;">
                                    <select name="duration_unit" id="duration_unit" class="form-control" style="flex: 1;">
                                        <option value="days" <?= $internship['duration_unit'] === 'days' ? 'selected' : '' ?>>Days</option>
                                        <option value="weeks" <?= $internship['duration_unit'] === 'weeks' ? 'selected' : '' ?>>Weeks</option>
                                        <option value="months" <?= $internship['duration_unit'] === 'months' ? 'selected' : '' ?>>Months</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3 col-6 mb-4">
                                <label class="form-label" for="mrp">Original MRP (₹)</label>
                                <input type="number" name="mrp" id="mrp" class="form-control" value="<?= htmlspecialchars($internship['mrp']) ?>" min="0" step="0.01" required oninput="calculateOGInfo()">
                            </div>
                            <div class="col-md-3 col-6 mb-4">
                                <label class="form-label" for="sales_price">Discounted Sales Price (₹)</label>
                                <input type="number" name="sales_price" id="sales_price" class="form-control" value="<?= htmlspecialchars($internship['sales_price']) ?>" min="0" step="0.01" required oninput="calculateOGInfo()">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-card">
                    ...
                    <div class="admin-body">
                        <div class="mb-4">
                            ...
                            <input type="file" name="internship_image" id="internship_image" class="form-control" accept="image/*" onchange="previewSingleImage(this, 'cover_preview_container', 'existing_cover_box')">
                            <div id="cover_preview_container" style="display: none;"></div>
                            <?php if (!empty($internship['internship_image'])): ?>
                                <div class="mt-2" id="existing_cover_box">
                                    <div class="image-preview-container animate__animated animate__fadeIn">
                                        <img src="<?= $project_base . htmlspecialchars($internship['internship_image']) ?>" class="image-preview-thumb">
                                        <div class="image-preview-info">
                                            <div class="image-preview-filename" style="color: #28a745; font-weight: 600;">Active Cover Image</div>
                                            <a href="<?= $project_base . htmlspecialchars($internship['internship_image']) ?>" target="_blank" class="text-success text-decoration-none font-weight-bold" style="font-size: 11px;">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="gallery-preview-grid" id="galleryGrid">
                                <?php 
                                $existing_gallery = json_decode($internship['gallery_images'] ?? '[]', true);
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-0">
                                ...
                                <select name="brochure_enabled" id="brochure_enabled" class="form-control" onchange="toggleBrochureUpload()">
                                    <option value="0" <?= $internship['brochure_enabled'] == 0 ? 'selected' : '' ?>>Disabled</option>
                                    <option value="1" <?= $internship['brochure_enabled'] == 1 ? 'selected' : '' ?>>Enabled (Require PDF upload)</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-0" id="brochureUploadBox" style="display: <?= $internship['brochure_enabled'] == 1 ? 'block' : 'none' ?>;">
                                ...
                                <?php if (!empty($internship['brochure_pdf'])): ?>
                                    <a href="<?= $project_base . $internship['brochure_pdf'] ?>" target="_blank" class="file-indicator-badge">
                ...
                <div class="admin-card">
                    ...
                    <div class="admin-body">
                        <div class="mb-4">
                            ...
                            <input type="text" name="meta_title" id="meta_title" class="form-control" value="<?= htmlspecialchars($internship['meta_title'] ?? '') ?>" placeholder="Web development internships Prayag" oninput="calculateOGInfo()">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="meta_description" id="meta_description" class="form-control" rows="3" placeholder="Join live project software engineering internship training at MG Education campus." style="resize: none;" oninput="calculateOGInfo()"><?= htmlspecialchars($internship['meta_description'] ?? '') ?></textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="meta_keywords" id="meta_keywords" class="form-control" value="<?= htmlspecialchars($internship['meta_keywords'] ?? '') ?>" placeholder="internship Prayagraj, software live project training">
                        </div>
                        ...
                        <div class="mb-0">
                            ...
                            <?php if (!empty($internship['featured_image'])): ?>
                                <div class="mt-2" id="existing_featured_box">
                                    <div class="image-preview-container animate__animated animate__fadeIn">
                                        <img src="<?= $project_base . htmlspecialchars($internship['featured_image']) ?>" class="image-preview-thumb">
                                        <div class="image-preview-info">
                                            <div class="image-preview-filename" style="color: #28a745; font-weight: 600;">Active Social OG Image</div>
                                            <a href="<?= $project_base . htmlspecialchars($internship['featured_image']) ?>" target="_blank" class="text-success text-decoration-none font-weight-bold" style="font-size: 11px;">
                                                <i class="fa-solid fa-up-right-from-square mr-1"></i>View Fullsize
                                            </a>
                                        </div>
                                    </div>
                ...
                <div class="mb-5">
                    <button type="submit" name="update_internship" class="btn-green-premium me-2">
                        <i class="fa-solid fa-floppy-disk"></i> Save Internship Changes
                    </button>
    ...
    function calculateOGInfo() {
        ...
        const schema = {
            ...
            "aggregateRating": {
                "@type": "AggregateRating",
                "ratingValue": "4.8",
                "ratingCount": "<?= $internship ? $internship['ratings_count'] : '24' ?>"
            }
        };
