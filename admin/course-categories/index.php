 ...
 * Course Categories Management Panel
 */
...
if (isset($_GET['delete_id'])) {
    try {
        ...
        // Delete the category (Cascade constraint handles Courses deletion automatically)
        $stmt = $db->prepare("DELETE FROM course_categories WHERE id = :id");
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    text: 'The course category was deleted from system records.',
...
try {
    ...
    if (!empty($search)) {
        $stmt = $db->prepare("SELECT * FROM course_categories WHERE name LIKE :search OR slug LIKE :search ORDER BY id DESC");
        $stmt->execute(['search' => "%{$search}%"]);
    } else {
        $stmt = $db->query("SELECT * FROM course_categories ORDER BY id DESC");
    }
    ...
    <div class="col-12">
        <div class="admin-card">
            <div class="admin-header">
                <h3>Course Categories Management</h3>
                            ...
                            <?php if (!empty($categories)): ?>
                                <?php foreach ($categories as $cat): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="text-muted" title="<?= htmlspecialchars($cat['description'] ?? '') ?>">
                                                <?= !empty($cat['description']) ? htmlspecialchars(substr($cat['description'], 0, 60)) . (strlen($cat['description']) > 60 ? '...' : '') : '<em class="text-secondary">No description</em>' ?>
                                            </span>
                                        </td>
                                        <td>
...
<script>
    function confirmDelete(id, name) {
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to delete the category "${name}". Note: ALL courses matching this category will be permanently deleted!`,
            icon: 'warning',
