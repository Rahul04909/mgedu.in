...
try {
    ...
    $stmt = $db->prepare("SELECT * FROM course_categories WHERE id = :id LIMIT 1");
    $stmt->execute(['id' => $cat_id]);
    $category = $stmt->fetch();
    
    if (!$category) {
        throw new Exception("The requested course category was not found.");
    }
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_category']) && $category) {
    try {
        ...
        $checkStmt = $db->prepare("SELECT id FROM course_categories WHERE slug = :slug AND id != :id LIMIT 1");
                ...
                <div class="admin-card">
                    ...
                    <div class="admin-body">
                        <div class="mb-4">
                            ...
                            <input type="text" name="meta_title" id="meta_title" class="form-control" value="<?= htmlspecialchars($category['meta_title'] ?? '') ?>" placeholder="Computer Science Wing - MG Education">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="meta_description" id="meta_description" class="form-control" rows="3" placeholder="Explore industry-oriented computer science and vocational software engineering courses." style="resize: none;"><?= htmlspecialchars($category['meta_description'] ?? '') ?></textarea>
                        ...
                        <div class="mb-0">
                            ...
                            <input type="text" name="meta_keywords" id="meta_keywords" class="form-control" value="<?= htmlspecialchars($category['meta_keywords'] ?? '') ?>" placeholder="computer science, courses, coding classes">
